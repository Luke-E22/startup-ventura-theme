...
<footer class="site-footer">
  ...
  <div class="wrap"><div class="footer-grid">
    <div class="footer-brand"><img src="${A}/img/logo-white.png" width="280" height="91" alt="Startup Ventura"><p>A 501(c)(3) nonprofit startup accelerator backing local founders in Ventura County.</p>${candidSeal('footer-seal')}</div>
...
<script src="${A}/js/main.js?v=11"></script>
</body></html>`;
...
const board = [
  { n: 'Luke Erickson', r: 'Board Member, Executive Director', p: 'team/luke-erickson.jpg', pos: 'center 22%', li: 'https://www.linkedin.com/in/luke-erickson/',
...
const boardGrid = (openBios = false) => `<div class="board-grid">${board.map(b => `<article class="board-card"><div class="board-card__media"><img class="board-card__photo" src="${A}/img/${b.p}" width="600" height="720"${(b.pos || b.zoom) ? ` style="${b.pos ? `object-position:${b.pos};` : ''}${b.zoom ? `transform:scale(${b.zoom});transform-origin:${b.pos || 'center'};` : ''}"` : ''} alt="${b.n}"></div><div class="board-card__body"><h3 class="board-card__name">${b.n}</h3><p class="board-card__role">${b.r}</p><details class="board-card__details"${openBios ? ' open' : ''}><summary>${openBios ? 'Bio' : 'Read bio'}</summary><p class="board-card__bio">${b.bio}</p></details><p class="board-card__links"><a href="${b.li}" target="_blank" rel="noopener">LinkedIn &nearr;</a></p></div></article>`).join('')}</div>`;
...
page('partner.html', {
  ...
  body: pageHead('Partner', 'Partner with us to keep talent here.', 'Cities, county offices, foundations, and companies all have a stake in keeping Ventura County&rsquo;s founders building here.') +
