...
const board = [
  ...
  // Sean Herwaldt — photo pending (high-res headshot not yet supplied); card
  // renders without an image until `p: 'team/sean-herwaldt.jpg'` is added.
  { n: 'Sean Herwaldt', r: 'Board Member, Director at Curri · SpaceX alum',
...
const boardGrid = (openBios = false) => `<div class="board-grid">${board.map(b => `<article class="board-card">${b.p ? `<div class="board-card__media"><img class="board-card__photo" src="${A}/img/${b.p}" width="600" height="720"${(b.pos || b.zoom) ? ` style="${b.pos ? `object-position:${b.pos};` : ''}${b.zoom ? `transform:scale(${b.zoom});transform-origin:${b.pos || 'center'};` : ''}"` : ''} alt="${b.n}"></div>` : ''}<div class="board-card__body"><h3 class="board-card__name">${b.n}</h3><p class="board-card__role">${b.r}</p><details class="board-card__details"${openBios ? ' open' : ''}><summary>${openBios ? 'Bio' : 'Read bio'}</summary><p class="board-card__bio">${b.bio}</p></details>${(b.li || b.ig || b.site) ? `<p class="board-card__links">${b.li ? `<a href="${b.li}" target="_blank" rel="noopener">LinkedIn &nearr;</a>` : ''}${b.ig ? `<a href="${b.ig}" target="_blank" rel="noopener">Instagram &nearr;</a>` : ''}${b.site ? `<a href="${b.site}" target="_blank" rel="noopener">${b.site.replace(/^https?:\/\/(www\.)?/, '')} &nearr;</a>` : ''}</p>` : ''}</div></article>`).join('')}</div>`;
